...
CSVViewer::CSVViewer(EvolutionCours *e, ChoixParam* p, QString c, QWidget *parent, QString mode): QWidget(parent), chemin(c), mode_affichage(mode){
    ...
    if(it == e->end()){
        ...
        if(mode_affichage == "manuel"){
            ...
            // on met le slot action dans les méthodes achat/vente afin d'incrémenter la date seulement lorsque l'achat ou la vente est permise (par exemple: quand nous avons assez d'argent)
...
void CSVViewer::remplissageBougiesPasAPas(){
    ...
    // ATTENTION à prendre celui et non celui de la bougie plate, qui sera faux car valeur arbitraire !
    achat_pap->setCours(it);
    vente_pap->setCours(it);
    valBougiePlate = it->getOpen();
    // On prend également la valeur Open de ce cours afin d'applatir toutes les bougies suivantes dont on ne doit pas connaître l'évolution au même stade
    // ce choix est fait afin de garder des valeurs plates aux alentours des valeurs précédentes pour rendre l'affichage plus lisible et ne pas avoir de valeurs trop divergentes avec celle par défaut.
    ...
    while(it!=eCSV->end()){
        ...
        // CREATION BOUGIES
        Bougie *bougie = new Bougie(valBougiePlate,valBougiePlate,valBougiePlate,valBougiePlate,&cours);
        categorieBougie << QString::number(it->getIndice() + 1);
        series->append(bougie);
        it++;
   }
...
void CSVViewer::vente(CoursOHLC *c, QDoubleSpinBox *m)
{
    ...
    if (nbTransactions==0){
        ...
        if (param->getContrepartie()>100){
            ...
            nbTransactions++;
            if (mode_affichage == "pasapas") action();
        }
        else
